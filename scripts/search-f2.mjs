...
function testF(cycles, returnDetails = false) {
  ...
  for (let i = 1; i <= 110; i++) {
    c = applyF(c)
    c = applyR(c)
    if (cubesEqual(c, s)) { frOrder = i; break }
  }
  ...
  for (let i = 1; i <= 110; i++) {
    c = applyF(c)
    c = applyU(c)
    if (cubesEqual(c, s)) { fuOrder = i; break }
  }
    ...
    for (const p1 of perms) {
      for (const p2 of perms) {
        for (const p3 of perms) {
          ...
          const score = (result.frComm ? 1 : 0) + (result.fuComm ? 1 : 0) + 
                        (result.frOrder === 105 ? 2 : 0) + (result.fuOrder === 105 ? 2 : 0) +
                        (result.tperm ? 10 : 0)
